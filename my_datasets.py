...
import utils
from utils import get_shuffled_data, load_hotelData_ToMem, loadDataToMem_2, get_overfit, get_masks, load_splits
...
class Metric_Dataset_Train(Dataset):
    ...
    def __triplet_getitem2__(self, index):
        ...
        if self.return_paths:
            return anch, pos, negs, paths # (B, C, H, W), (B, C, H, W), (B, NN, C, H, W), [anch_path, pos_path, neg1_path, ...]
        else:
            return anch, pos, negs
...
class DB_Dataset(Dataset):
    def __init__(self, args, transform=None, mode='test', return_pairs=False):
        ...
        self.all_shuffled_data = get_shuffled_data(self.datas,
                                                   seed=args.seed,
                                                   one_hot=False,
                                                   both_seen_unseen=False,
                                                   shuffle=False)
...
class Metric_Dataset_Train_New(Dataset):
    def __init__(self, args, transform=None, mode='', save_pictures=False, overfit=False, return_paths=False,
                 batchhard=[False, 0, 0], allow_same_chain_negative=True, is_train=False, main_split=None, backgroud_splits=[]):
        super(Metric_Dataset_Train_New, self).__init__()
        self.fourth_dim = args.fourth_dim
        np.random.seed(args.seed)
        self.transform = transform
        self.save_pictures = save_pictures
...
class Local_Feat_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x1_global, x2_global = torch.tensor(self.__add_dims(global_pair[0])), torch.tensor(self.__add_dims(global_pair[1]))
...
class BaseDataSet(Dataset):
    ...
    def __init__(self, img_source, transforms=None, mode="RGB"):
        ...
        self.num_classes = len(list(self.datas.keys()))
